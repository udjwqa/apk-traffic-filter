...
export function useAuditLog() {
  ...
  const fetchPage = useCallback(async (p: number, f: AuditLogFilters) => {
    setIsLoading(true);
    try {
      ...
      setEntries(result.entries || result.data || []);
  ...
  const updateFilters = useCallback((partial: Partial<AuditLogFilters>) => {
  ...
  const handleBanIp = useCallback(async (ip: string) => {
    await fetch(`${API_URL}/api/bans/honeypot`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ip, reason: "manual_ban" }),
    });
    return { success: true };
  }, []);
